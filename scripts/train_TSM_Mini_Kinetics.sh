...
# Parameters!
mainFolder="net_runs"
subFolder="TSM_MS_R18_mini_kinetics_run4"
snap_pref="MotionSqueeze"

train_path="data/Mini-kinetics_rgb_train.txt"
val_path="data/Mini-kinetics_rgb_val.txt"


#############################################
#--- training hyperparams ---
dataset_name="minikinetics"
netType="MS"
batch_size=48
learning_rate=0.01 # lr follows root of bs
num_segments=8
mode=0
dropout=0.5
iter_size=1
...
python3 -u main_kinetics.py ${dataset_name} RGB ${train_path} ${val_path}  --arch ${netType} --num_segments ${num_segments} --mode ${mode} --gd 20 --wd 1e-4 --lr ${learning_rate} --lr_steps 20 40 --epochs 50 -b ${batch_size} -i ${iter_size} -j ${num_workers} --dropout ${dropout} --snapshot_pref ${mainFolder}/${subFolder}/${snap_pref} --consensus_type avg --eval-freq 1 --rgb_prefix img_ --pretrained_parts finetune --no_partialbn -p 20 --val_output_folder ${mainFolder}/${subFolder}/validation --resume ${mainFolder}/${lastCheckpoint} 2>&1 | tee -a ${mainFolder}/${subFolder}/training/log.txt    

else
     echo "Training with initialization"
     echo $(pwd)
python3 -u main_kinetics.py ${dataset_name} RGB ${train_path} ${val_path} --arch ${netType} --num_segments ${num_segments} --mode ${mode} --gd 20 --wd 1e-4 --lr ${learning_rate} --lr_steps 20 40 --epochs 50 -b ${batch_size} -i ${iter_size} -j ${num_workers} --dropout ${dropout} --snapshot_pref ${mainFolder}/${subFolder}/${snap_pref} --consensus_type avg --eval-freq 1 --rgb_prefix img_ --pretrained_parts finetune --no_partialbn -p 20 2>&1 | tee -a ${mainFolder}/${subFolder}/training/log.txt
